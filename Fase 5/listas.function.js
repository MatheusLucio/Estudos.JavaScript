...
function stringConcatenadaComReduce(listas) {}
...
// substituir a linha 62 por um reduce
function soma2NumerosArray(lista, num1, num2) {
  const verificar = lista.filter(
...
// 9. Dado um array de números, retorne um novo array contendo a média de dois elementos específicos de todos os números,
// onde o valor de um elemento é maior que um determinado valor.
...
function maiorNumeroIndex(lista) {
  let valor = 0;
  const num = lista.forEach(function (numero) {
    if (numero > valor) {
      valor = numero;
    }
  });

  lista.findIndex(function (numero) {
    return numero === valor;
  });
  return 'Medio 22 = ' + num;
}
...
function segundoMenorIndex(lista) {
  return (
    'Medio 23 = ' + lista.findIndex((numero) => numero === lista.sort()[1])
  );
}
...
function maiorFrequencia(lista) {
  ...
  return (
    'Muito Dificl 24 = ' +
    lista.findIndex((numero) => numero === numeroMaisFrequente)
  );
}
...
function numerosPrimos3(lista) {
  const primo = lista.findIndex(function (numero) {
    if (numero < 2) return false;
    for (let i = 2; i < numero; i++) {
      if (numero % i === 0) {
        return false;
...
function primeiraString(lista) {
  const valorRemovido = lista.splice(0, 1);
  console.log(lista);
  return 'Facil 29 = ' + valorRemovido;
}
